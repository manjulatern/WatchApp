...
class AntiquorumSpider(scrapy.Spider):
    ...
    def parse(self, response):
        total_lots = response.xpath('//div[@class="container"]/div[@class="row"]/div[@class="col-md-3"]/h4/text()').extract_first().strip().split(" ")[0]
        
        logging.warn("AntiquorumSpider; msg=Spider started;url= %s",response.url)
        lots_per_page = 20
        total_pages = math.ceil(int(total_lots)/lots_per_page)
        logging.warn("AntiquorumSpider; Total Pages = %s ; URL: %s;",total_pages,response.url)
    ...
    def parse_urls(self,response):
        ...
        url_list = products.xpath('//div[@class="shadow mt-4"]/div[@class="row"]/div[@class="N_lots_thumbail col-sm p-2"]/div[@class="mt-4"]/a/@href').extract()
        ...
        logging.debug("AntiquorumSpider; msg=URLs to be Scraped %s;url= %s",len(final_url),response.url)
    ...
    def parse_item(self,response):
        item = WatchItem()
        try:
            all_desc = response.xpath('//div[@class="container"]/div[@class="container"]/div/div[1]/div[2]')
            ...
            date_location = all_desc.xpath("//p[3]/text()").extract_first() or None
            ...
            lot = all_desc.xpath('//h3/text()').extract_first().strip().split(" ") or None
            lot_number = re.findall(r'\d+',lot[1])[0]
            item['lot'] = lot_number
            
            images_url = response.xpath('//div[@class="container"]/div[@class="container"]/div/div/div[1]/div[2]/a/@href').extract() or None
            item['images'] = images_url
            
            title = all_desc.xpath('//strong/p/text()').extract_first()
            if not title:
                title = all_desc.xpath('//strong/div/font/strong/text()').extract_first()
            ...
            raw_table_data = all_desc.xpath('div[@class="row"]/div/div/div/table').extract()
            table_data = ""
            if raw_table_data:
                table_data = "Table:: " + re.sub("\t","",re.sub("\n","",raw_table_data[0]))
                
            desciption = description + table_data
    
            notes = response.xpath('//div[@class="container"]/div[@class="container"]/div/div[2]/div/text()').extract()
            if notes:
                descrption = desciption + notes[0]
            
            item['description'] = description
            
            min_max_price = all_desc.xpath("//h4[1]/text()").extract_first().strip().split(" ")
            ...
            logging.info("AntiquorumSpider; msg=Crawling Processed;url= %s",response.url)
        except Exception as e:
            item['status'] = "Failed"
            logging.error("AntiquorumSpider; msg=Crawling Failed;url= %s",response.url)
            logging.debug("AntiquorumSpider; msg=Crawling Failed;url= %s;Error=%s",response.url,traceback.format_exc())
