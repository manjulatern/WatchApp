...
class PhillipsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        logging.warn("PhillipsSpider; msg=Spider started;url= %s",response.url)
        all_lots = response.xpath('//div[@class="auction-page-grid"]/ul/li[@class="lot single-cell"]/div[@class="phillips-lot"]')
        auction_details = response.xpath('//div[@class="auction-details"]')
        total_lots = response.xpath('//nav[@class="sort-nav"]/div[1]/span/text()').extract()[0]
        logging.debug("PhillipsSpider; msg=Total Lots: %s;url= %s",len(all_lots),response.url)
        logging.debug("PhillipsSpider; msg=Total Lots: %s;url= %s",total_lots,response.url)
        #2 Auction Name
        name = response.xpath('//div[@class="sale-title-banner"]/a/strong/text()').extract()[0]

        date_location = response.xpath('//div[@class="sale-title-banner"]/a/span/text()').extract()[0].split(" ")
        ...
        for lot in all_lots:
            url_info = lot.xpath('div[@class="image"]/a[@class="detail-link"]/@href').extract()
            if url_info:
    ...
    def parse_details(self,response):
        ...
        try:
            ...
            #5 Lot
            lot_number_info = lot.xpath('a/span/p/text()').extract()[0]
            lot_number =  re.findall(r'\d+',lot_number_info)[0]
            item["lot"] = lot_number
            artist = lot.xpath('a/p/text()').extract()[0]
            ...
            short_desc = response.xpath('//div[@class="lot-information"]/p[@class="lot-detail-header__title"]/em').extract()[0]
            description =  description + "\n"
            desc = response.xpath('//ul[@class="info-list"]/li[1]/p').extract()[0]
            description = description + desc
            essay_info = response.xpath('//div[@class="lot-essay"]/p/text()').extract()
            essay = ""
            ...
            estimation = lot.xpath('a/p[@class="phillips-lot__description__estimate"]/text()').extract()
            ...
            sold_price_info = lot.xpath('a/p[@class="phillips-lot__sold"]/text()').extract()
            sold_price = sold = 0
            lot_currency= ""
            if len(sold_price_info) > 1:
                sold = 1
                sold_price = sold_price_info[2].replace(",","")
            ...
            item['status'] = "Failed"
            logging.error("PhillipsSpider; msg=Crawling Failed > %s;url= %s",str(e),response.url)
            logging.debug("PhillipsSpider; msg=Crawling Failed;url= %s;Error=%s",response.url,traceback.format_exc())
