...
class WatchscrapyPipeline(object):
    ...
    def close_spider(self, spider):
        logging.warn("WatchExtraction; msg=All Data Extraction  has been Completed;")
    ...
    def process_item(self, item, spider):
        ...
        try:
            ...
            prev_auction = Auction.objects.filter(url=item["auction_url"]).first()
            ...
            if prev_auction:
                ...
                auction.date = datetime.strptime(item["date"].strip(),'%b %d,%Y').strftime('%Y-%m-%d')
            ...
            if item["lot_currency"] == "N/A":
                ...
                sold_price_usd = self.get_usd_amount(item["lot_currency"],item["sold_price"])
            lot.sold_price_dollar = sold_price_usd

            lot.auction_id = auction_id
            lot.save()
            logging.info("WatchExtraction; msg=Processing Completed; url= %s",item["url"])
        except Exception as e:
            logging.error("WatchExtraction; msg=Processing Failed > %s; url= %s",str(e),item["url"])
            logging.error("WatchExtraction; msg=Processing Failed; url= %s; Error: %s",item["url"],traceback.format_exc())
        return item
