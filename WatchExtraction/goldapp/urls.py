...
urlpatterns = [
		...
		re_path(r'^platinum_data/(?P<platinumgram>\d+(?:\.\d+)?)/(?P<percentage>\d+(?:\.\d+)?)', views.platinum_data, name='platinum_data'),
		re_path(r'^silver_data/(?P<carat>\w+?)/(?P<silvergram>\d+(?:\.\d+)?)/(?P<percentage>\d+(?:\.\d+)?)', views.silver_data, name='silver_data'),
]