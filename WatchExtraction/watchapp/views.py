...
def index(request):
    ...
    lot_list = Lot.objects.all().prefetch_related("auction").order_by('auction__date').reverse()
...
def advsearch(request):
    ...
    if from_date and to_date:
        lot_list = Lot.objects.all().prefetch_related("auction").filter(auction__date__range=[from_date,to_date])
        print(lot_list)
        any_filter = True
        url_parameters = url_parameters + "&from_date="+from_date+"&to_date="+to_date
    #ACA FILTRO POR MONEDA
    if option=="currency":
        lot_list = Lot.objects.filter(lot_currency__icontains=search).prefetch_related("auction")
        any_filter = True
    #ACA FILTRO POR PRECIO
    if option=="sold_price":
        lot_list = Lot.objects.filter(sold_price__icontains=search).prefetch_related("auction")
        any_filter = True
    #ACA FILTRO POR ESTIMADO
    if option=="estimate_min_price":
        lot_list = Lot.objects.filter(estimate_min_price__icontains=search).prefetch_related("auction")
        any_filter = True
    if option=="estimate_max_price":
        lot_list = Lot.objects.filter(estimate_max_price__icontains=search).prefetch_related("auction")
        any_filter = True
...
def allJobs(request):
    ...
    job_list = Job.objects.filter(status__icontains="In Progress").prefetch_related("auction_house").order_by('id')
...
def completedJobs(request):
    ...
    job_list = Job.objects.filter(status__icontains="Completed").prefetch_related("auction_house").order_by('id')
...
def failedJobs(request):
    ...
    job_list = Job.objects.filter(status__icontains="Failed").prefetch_related("auction_house").order_by('id')
...
def job_details(request,job):
    ...
    for url in urls.split(","):
        counts = {}
        lot = Lot.objects.all().prefetch_related("auction").filter(auction__url=url.strip())
...
def houses(request):
    ...
    all_house_info = []
    lots = Lot.objects.values('auction__auction_house','auction__auction_house__name').annotate(Count('lot_number'))
    lot_info = {}
    for lot in lots:
        lot_info[lot['auction__auction_house']] = lot['lot_number__count']

    auc_houses = AuctionHouse.objects.values('name','pk').annotate(Count('auction__name'),Sum('auction__actual_lots'))
...
def house_details(request,house):
    ...
    auctions_list = Auction.objects.filter(auction_house=house).annotate(Count('lot')).order_by('date').reverse()
